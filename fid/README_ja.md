...
グローバルオプションをすべてのジョブに適用する場合は `--` 以降に追加します。
...
- `--jobs-base` を指定すると、相対パスで記述された `real_dir` / `gen_dir` をそのベースパスから解決できます。
- `jobs` エントリごとに以下のフィールドを個別に設定できます:
  - `cem_backbones`: 使用するバックボーンのリスト（`["cem500k"]`, `["cem1.5m"]`, `["cem500k", "cem1.5m"]` など）
  - `cem_weights`: カスタムの重みファイルパス（単一バックボーン時のみ）
  - `script_args`: スイートスクリプトに渡す追加 CLI オプション（`--` の前に挿入される）
  - `extra_args`: Python スクリプトに渡すオプション（`--` 以降に渡される）
- `--stop-on-error` や `--dry-run`、`--json-log`、`--quiet` などの制御オプションも用意しています。詳細は `python fid/run_fid_suite_batch.py --help` を参照してください。
...
> **注意**: `run_fid_suite_venv.sh` は Docker 版と異なり、`--cem-backbone` を **1 回しか指定できません**（複数バックボーンを一括評価する場合はスクリプトを複数回実行するか、Docker 版またはバッチ処理ヘルパーを使用してください）。
